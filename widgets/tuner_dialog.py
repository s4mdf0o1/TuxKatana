...
def rgb_for_diff(diff_hz, span):
    ...
    if t <= -0.5:
        s = (t + 1.0) / 0.5
        r, g, b = 0, lerp(0, 255, s), 255
    elif t <= 0.0:
        s = (t + 0.5) / 0.5
        r, g, b = 0, 255, lerp(255, 0, s)
    elif t <= 0.5:
        s = (t - 0.0) / 0.5
        r, g, b = lerp(0, 255, s), 255, 0
    else:
        s = (t - 0.5) / 0.5
        r, g, b = 255, lerp(255, 0, s), 0
    return r, g, b

def ansi_truecolor(r,g,b): return f"<span foreground='#{r:02x}{g:02x}{b:02x}'>"

def freq_to_bar_pango(current, target, width=BAR_WIDTH, span=BAR_SPAN):
    ...
    r,g,b = rgb_for_diff(cur_diff, span/2)
    cursor_col = ansi_truecolor(r,g,b)
...
def audio_callback(indata, frames, time_info, status):
    ...
    with buf_lock:
        f = len(mono)
        if f >= WINDOW_SIZE: buffer[:] = mono[-WINDOW_SIZE:]
        else:
            buffer[:-f] = buffer[f:]
            buffer[-f:] = mono

def processing_thread(dialog=None):
    global detected_freq, prev_freq, stop_flag
    while not stop_flag:
        time.sleep(HOP_TIME)
        with buf_lock: x = buffer.copy()
        if np.max(np.abs(x)) < 1e-8: continue
        x -= np.mean(x); rms = np.sqrt(np.mean(x*x))
        if rms < RMS_THRESHOLD:
            if dialog:
                GLib.idle_add(dialog.update_display, "--", 0.0, 0.0, " " * BAR_WIDTH)
        ...
        if freq <= 0 or not isfinite(freq):
            ...
            continue
        out = freq if prev_freq==0.0 else SMOOTH_ALPHA*freq+(1.0-SMOOTH_ALPHA)*prev_freq
        prev_freq = out; detected_freq = out
        if dialog:
            note,target = get_closest_string(out)
            offset = out - target
            bar = freq_to_bar_pango(out, target)
            GLib.idle_add(dialog.update_display, note, out, offset, bar)

def find_katana_device():
...
class TunerDialog(Gtk.Dialog):
    ...
    def update_display(self, note, freq, offset, bar_markup):
        r,g,b = rgb_for_diff(offset, BAR_SPAN/2)
